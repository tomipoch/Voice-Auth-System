...
class ChallengeService:
    ...
    async def create_challenge_batch(
        self, user_id: UserId, count: int = 3, difficulty: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if len(phrases) < count:
            ...
            logger.warning(
                f"Only {len(phrases)} unique phrases available, need {count}. Allowing repetition."
            )
            phrases = await self._phrase_repo.find_random(
                user_id=user_id,
                count=count,
        ...
        for phrase in phrases:
            ...
            timeout_seconds = CHALLENGE_TIMEOUT.get(
                phrase.difficulty, 90
    ...
    async def create_challenge(
        self, user_id: UserId, difficulty: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        batch = await self.create_challenge_batch(
            user_id=user_id, count=1, difficulty=difficulty
        )
    ...
    async def validate_challenge_strict(
        self, challenge_id: ChallengeId, user_id: UserId
    ) -> tuple[bool, str]:
        """
        ...
        if challenge.get("user_id") != user_id:
            await self._audit_repo.log_event(
                ...
                metadata={
                    "error": "user_mismatch",
                    "expected_user": str(challenge.get("user_id")),
                },
            )
    ...
    async def cleanup_expired_challenges(self) -> int:
        ...
        cleanup_hours = int(
            await self._rules_service.get_rule_value(
                "cleanup_expired_after_hours", default=1.0
            )
        )
    ...
    async def cleanup_used_challenges(self) -> int:
        ...
        cleanup_hours = int(
            await self._rules_service.get_rule_value(
                "cleanup_used_after_hours", default=24.0
            )
        )
        deleted_count = await self._challenge_repo.cleanup_used_challenges(
            older_than_hours=cleanup_hours
        )
    ...
    async def analyze_phrase_performance(self) -> Dict[str, Any]:
        ...
        min_success_rate = await self._rules_service.get_rule_value(
            "min_success_rate", default=0.70
        )
        min_asr_score = await self._rules_service.get_rule_value(
            "min_asr_score", default=0.80
        )
        min_phrase_ok_rate = await self._rules_service.get_rule_value(
            "min_phrase_ok_rate", default=0.75
        )
        min_attempts = int(
            await self._rules_service.get_rule_value(
                "min_attempts_for_analysis", default=10.0
            )
        )
        ...
        logger.info(
            f"Analyzing phrase performance with thresholds: success_rate={min_success_rate}, asr={min_asr_score}, phrase_ok={min_phrase_ok_rate}"
        )
    ...
    async def _check_rate_limits(self, user_id: UserId) -> None:
        ...
        max_active = int(
            await self._rules_service.get_rule_value(
                "max_challenges_per_user", default=9.0
            )
        )  # Increased from 3 to 9
        max_per_hour = int(
            await self._rules_service.get_rule_value(
                "max_challenges_per_hour", default=20.0
            )
        )
        ...
        recent_count = await self._challenge_repo.count_recent_challenges(
            user_id, since_hours=1
        )
