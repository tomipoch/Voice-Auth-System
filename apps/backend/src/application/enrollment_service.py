...
class EnrollmentService:
    ...
    async def start_enrollment(
        ...
        if voiceprint_exists and force_overwrite:
            ...
            logger.info(
                f"Deleted existing voiceprint for user {user_id} (force_overwrite=True)"
            )

        # Create challenges for enrollment (batch operation)
        challenges = await self._challenge_service.create_challenge_batch(
            user_id=user_id, count=MIN_ENROLLMENT_SAMPLES, difficulty=difficulty
        )

        if len(challenges) < MIN_ENROLLMENT_SAMPLES:
            raise ValueError(
                f"Not enough challenges created. Need {MIN_ENROLLMENT_SAMPLES}"
            )
    ...
    async def add_enrollment_sample(
        ...
        challenge_found = any(
            c["challenge_id"] == challenge_id for c in session.challenges
        )
    ...
    async def complete_enrollment(
        self, enrollment_id: UUID, speaker_model_id: Optional[int] = None
    ) -> Dict:
        ...
        embeddings = [
            ...
            for sample in samples[-MIN_ENROLLMENT_SAMPLES:]
        ]
        average_embedding = np.mean(embeddings, axis=0)
        average_embedding = average_embedding / np.linalg.norm(average_embedding)

        # Check if user already has a voiceprint
        existing_voiceprint = await self._voice_repo.get_voiceprint_by_user(
            session.user_id
        )
    ...
    def _calculate_enrollment_quality(self, embeddings: List[np.ndarray]) -> float:
        ...
        for i in range(len(embeddings)):
            for j in range(i + 1, len(embeddings)):
                sim = self._biometric_validator.calculate_similarity(
