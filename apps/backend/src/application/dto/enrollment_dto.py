...
class StartEnrollmentResponse(BaseModel):
    ...
    voiceprint_exists: bool = Field(
        False, description="Whether user already has a voiceprint"
    )
...
class AddEnrollmentSampleResponse(BaseModel):
    ...
    next_phrase: Optional[dict] = Field(
        None, description="Next phrase to read (if not complete)"
    )
    quality_score: Optional[float] = Field(
        None, description="Audio quality score (0-1)"
    )
...
class CompleteEnrollmentResponse(BaseModel):
    ...
    enrollment_quality: float = Field(
        ..., description="Overall enrollment quality (0-1)", ge=0, le=1
    )
...
class EnrollmentStatusResponse(BaseModel):
    ...
    status: str = Field(
        ..., description="user_not_found | enrolled | in_progress | not_started"
    )
    voiceprint_id: Optional[str] = Field(
        None, description="Voiceprint ID (when enrolled)"
    )
    created_at: Optional[str] = Field(
        None, description="Voiceprint creation timestamp (ISO 8601)"
    )
    samples_count: int = Field(
        0, description="Samples collected in the current session"
    )
    required_samples: int = Field(
        0, description="Samples required to complete enrollment"
    )
    phrases_used: List[dict] = Field(
        default_factory=list, description="Phrases already read in this session"
    )
