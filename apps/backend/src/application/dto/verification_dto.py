...
class StartVerificationResponse(BaseModel):
    ...
    verification_id: str = Field(
        ..., description="Unique session ID for this verification"
    )
    user_id: str = Field(..., description="User ID being verified")
    phrase: dict = Field(
        ..., description="Phrase to read (contains id, text, difficulty)"
    )
...
class VerifyVoiceResponse(BaseModel):
    ...
    similarity_score: float = Field(
        ..., description="Voice similarity to enrolled voiceprint (0-1)", ge=0, le=1
    )
    anti_spoofing_score: Optional[float] = Field(
        None, description="Probability of genuine speech (0-1)"
    )
    phrase_match: Optional[bool] = Field(
        None, description="Whether spoken text matched expected phrase"
    )
    is_live: bool = Field(..., description="Whether anti-spoofing passed")
    threshold_used: float = Field(
        ..., description="Similarity threshold used for decision"
    )
...
class VerifyPhraseResponse(BaseModel):
    ...
    phrase_number: int = Field(
        ..., description="Which phrase was verified (1, 2, or 3)"
    )
