...
class PhraseQualityRulesService:
    ...
    async def get_rules_by_type(self, rule_type: str) -> List[Dict[str, Any]]:
        ...
        if rule_type not in ["threshold", "rate_limit", "cleanup"]:
            raise ValueError(
                f"Invalid rule_type: {rule_type}. Must be 'threshold', 'rate_limit', or 'cleanup'"
            )

        return await self._rules_repo.get_rules_by_type(rule_type)

    async def update_rule(
        self, rule_name: str, new_value: float, admin_id: Optional[UUID] = None
    ) -> bool:
        """
        ...
        if rule_name in ["min_success_rate", "min_asr_score", "min_phrase_ok_rate"]:
            if not (0.0 <= new_value <= 1.0):
                raise ValueError(
                    f"Percentage values must be between 0.0 and 1.0, got: {new_value}"
                )
        ...
        if success:
            ...
            logger.info(
                f"Rule '{rule_name}' updated to {new_value} by admin {admin_id}"
            )
    ...
    async def get_threshold_rules(self) -> Dict[str, float]:
        ...
        return {rule["rule_name"]: float(rule["rule_value"]["value"]) for rule in rules}
    ...
    async def get_rate_limit_rules(self) -> Dict[str, int]:
        ...
        return {rule["rule_name"]: int(rule["rule_value"]["value"]) for rule in rules}
