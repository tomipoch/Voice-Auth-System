...
class VerificationService:
        ...
        *,
        ...
        if audio_bytes is not None:
            ...
            if policy and policy.get("keep_audio"):
                audio_id = await self._attempt_repo.save_audio_blob(
    ...
    def _is_verification_passed(
        self, similarity_score: float, is_live: bool, phrase_match: bool
    ) -> bool:
        """Determine if verification passed based on all checks."""
        return (
            similarity_score >= self._similarity_threshold and is_live and phrase_match
        )

    def _get_phrase_match_result(
        self, transcribed_text: Optional[str], expected_phrase: Optional[str]
    ) -> tuple[float, bool]:
    ...
    async def start_verification(
        self, user_id: UUID, difficulty: str = "medium"
    ) -> Dict:
    ...
    async def verify_voice(
        ...
        is_valid, reason = await self._challenge_service.validate_challenge_strict(
            challenge_id=challenge_id, user_id=session.user_id
        )
        ...
        similarity_score = self._biometric_validator.calculate_similarity(
        ...
        # Check anti-spoofing
        is_live = (
            anti_spoofing_score is None
            or anti_spoofing_score < self._anti_spoofing_threshold
        )
        ...
        is_verified = self._is_verification_passed(
            similarity_score, is_live, phrase_match
        )
        ...
        await self._audit_repo.log_event(
            ...
            metadata={
                ...
                "anti_spoofing_score": (
                    float(anti_spoofing_score) if anti_spoofing_score else None
                ),
    ...
    async def get_verification_history(self, user_id: UUID, limit: int = 10) -> Dict:
    ...
    ) -> Optional[MultiPhraseVerificationSession]:
    ...
    async def start_multi_phrase_verification(
        ...
        challenges = await self._challenge_service.create_challenge_batch(
            user_id=user_id, count=3, difficulty=difficulty
        )

        if len(challenges) < 3:
            raise ValueError(
                "Not enough challenges created for multi-phrase verification"
            )

        # Create verification session
        verification_id = uuid4()
        session = MultiPhraseVerificationSession(user_id, verification_id, challenges)
    ...
    async def verify_phrase(
        ...
        asr_confidence = (
            self._calculate_phrase_similarity(expected_phrase, transcribed_text)
        ...
        spoof_prob = (
            float(anti_spoofing_score) if anti_spoofing_score is not None else 1.0
        )

        # Calculate Final Composite Score using helper
        final_score = self._calculate_composite_score(
            similarity_score, spoof_prob, asr_confidence
        )
        ...
        if is_complete:
            ...
            await self._persist_attempt(
                ...
                accept=is_verified,
                reason=(
                    "spoof" if any_spoof else "ok" if is_verified else "low_similarity"
                ),
                ...
                policy_id="multi",
                challenge_id=(
                    UUID(str(session.challenges[0]["challenge_id"]))
