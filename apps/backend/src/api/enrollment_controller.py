...
async def add_enrollment_sample(
    ...
    quality_info = voice_engine.validate_audio_quality(
    ...
    try:
        ...
        if wav_bytes and session:
            ...
            logger.info(
                f"Saved enrollment audio for user {user.get('email') if user else session.user_id}, sample {sample_result['samples_completed']}"
            )
        else:
    ...
    return AddEnrollmentSampleResponse(
        ...
        next_phrase=sample_result.get(
            "next_challenge"
        ),  # Map to next_phrase for compatibility
...
async def complete_enrollment(
    ...
    result = await enrollment_service.complete_enrollment(
        enrollment_id=enrollment_uuid, speaker_model_id=speaker_model_id
    )
