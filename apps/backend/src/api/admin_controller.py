...
def _filter_logs_by_company(
    logs: list, company_user_ids: set, company_emails: set
) -> list:
    """Filter logs to only include those belonging to company users."""
    return [
        log
        for log in logs
        if _belongs_to_company(log, company_user_ids, company_emails)
    ]
...
def _transform_log_to_activity(log, default_timestamp) -> dict:
    ...
    metadata = _parse_metadata(log.get("metadata", {}))
    details = (
        metadata.get("message", "") if isinstance(metadata, dict) else str(metadata)
    )
    if not details:
        ...
        transformed_users.append(
            {
                ...
                "enrollment_status": (
                    "enrolled" if user.get("has_voiceprint", False) else "pending"
                ),
...
async def get_user_details(
    ...
    if (
        ...
        and user.get("company") != current_user["company"]
    ):
        raise HTTPException(
    ...
    if user.get("has_voiceprint"):
        ...
        try:
            ...
            logger.warning(
                f"Failed to fetch voiceprint details for user {user_id}: {e}"
            )
...
async def get_system_stats(
    ...
    recent_logs = await audit_repo.get_logs(
        start_time=last_7_days, limit=10000  # High limit to get all recent logs
    )
    ...
    if current_user["role"] == "admin":
        ...
        recent_logs = _filter_logs_by_company(
            recent_logs, company_user_ids, company_emails
        )
    ...
    for log in recent_logs:
        if (
            log.get("action") in verification_actions
            and log.get("entity_type") in verification_types
        ):
    ...
    daily_verifications = [
        {"date": date, "count": count} for date, count in sorted(daily_stats.items())
    ]
    ...
    if all_verification_logs:
        all_successful = [
            log for log in all_verification_logs if log.get("success", False)
        ]
...
async def get_recent_activity(
    ...
    try:
        ...
        logs = await audit_repo.get_logs(
            action=action, limit=limit * 10  # Get more to filter by company
        )
        ...
        if current_user["role"] == "admin":
            ...
            for log in logs:
                ...
                if actor in company_user_ids or (
                    "@" in actor
                    and any(str(u.get("email")) == actor for u in company_users)
                ):
...
async def get_phrase_quality_rules(
    ...
    current_user: dict = Depends(require_admin),
    rules_service: PhraseQualityRulesService = Depends(
        get_phrase_quality_rules_service
    ),
):
    """
...
async def update_phrase_quality_rule(
    ...
    try:
        ...
        success = await rules_service.update_rule(
            rule_name=rule_name, new_value=request.value, admin_id=admin_id
        )
...
async def create_api_client(
    ...
    try:
        client_id, raw_key = await client_repo.create_client(
            request.name, request.contact_email
        )
        return CreateClientResponse(
            client_id=str(client_id), name=request.name, api_key=raw_key
        )
    except Exception as e:
...
@admin_router.post(
    "/clients/{client_id}/rotate", response_model=RotateClientKeyResponse
)
