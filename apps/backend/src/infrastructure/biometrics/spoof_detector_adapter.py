...
class SpoofDetectorAdapter:
    ...
    def __init__(
        self,
        model_id: int = 2,
        model_name: str = "ensemble_antispoofing",
        use_gpu: bool = True,
    ):
    ...
    def _load_antispoofing_models(self):
        ...
        try:
            ...
            if success_count > 0:
                ...
                logger.info(
                    f"Anti-spoofing ensemble loaded: {success_count}/2 models available"
                )
            else:
    ...
    def _load_speechbrain_model(self, model_key: str):
        ...
        try:
            ...
            if not hyperparams_file.exists():
                logger.warning(
                    "SpeechBrain model %s incomplete at %s", model_key, model_path
                )
                return None

            classifier = EncoderClassifier.from_hparams(
                source=str(model_path), run_opts={"device": str(self.device)}
            )
    ...
    def _get_model_prediction(self, model_name: str, waveform: torch.Tensor) -> dict:
        ...
        try:
            if local_model:
                score = local_model.predict_spoof_probability(
    ...
    def get_spoof_details(self, audio_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            confidence = self._calculate_ensemble_confidence(
                individual_scores, spoof_prob
            )

            # Classify attack types based on model strengths
            attack_probabilities = self._classify_attack_types(
                individual_scores, spoof_prob
            )
            ...
            return {
                ...
                "models_available": {
                    ...
                    or "aasist" in self._local_models,
                    "rawnet2": self._rawnet2_model is not None
                    or "rawnet2" in self._local_models,
                },
            }
    ...
    def _calculate_ensemble_confidence(
        self, individual_scores: Dict[str, float], ensemble_score: float
    ) -> float:
        """Calculate confidence based on model agreement."""
