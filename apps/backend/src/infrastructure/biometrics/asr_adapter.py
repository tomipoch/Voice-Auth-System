...
class ASRAdapter:
            ...
            try:
                if not model_manager.is_model_available(self._model_name):
                    ...
                    if not download_success:
                        logger.warning(
                            f"ASR model '{self._model_name}' download failed, using fallback"
                        )
                        self._model_loaded = False
                        return
                ...
                required_files = ["hyperparams.yaml", "wav2vec2.ckpt", "tokenizer.ckpt"]

                all_files_exist = all((asr_path / f).exists() for f in required_files)

                if not all_files_exist:
                    logger.warning(
                        f"ASR model incomplete at {asr_path}, using fallback"
                    )
                    self._model_loaded = False
                    return
                ...
                try:
                    ...
                    logger.info(
                        "Lightweight ASR model loaded successfully from local path"
                    )
    ...
    def _enhanced_mock_transcription(
        self, audio_data: bytes, waveform: torch.Tensor
    ) -> str:
    ...
    def verify_phrase(self, audio_data: bytes, expected_phrase: str) -> Dict[str, Any]:
        ...
        try:
            ...
            similarity = self.calculate_phrase_similarity(
                expected_phrase, recognized_text
            )
            ...
            details = {
                ...
                "word_order_preserved": self._check_word_order(
                    expected_phrase, recognized_text
                ),
                "key_words_present": self._check_key_words(
                    expected_phrase, recognized_text
                ),
            }
            ...
            return {
                ...
                "details": details,
                "asr_model_used": (
                    self._model_name if self._model_loaded else "fallback"
                ),
            }
    ...
    def _check_key_words(self, expected: str, recognized: str) -> Dict[str, bool]:
        ...
        for category, words in key_categories.items():
            category_present = bool(words.intersection(expected_words)) and bool(
                words.intersection(recognized_words)
            )
    ...
    def calculate_phrase_similarity(self, expected: str, recognized: str) -> float:
        ...
        similarity = difflib.SequenceMatcher(
            None, expected_norm, recognized_norm
        ).ratio()
