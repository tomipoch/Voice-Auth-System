...
class SpeakerEmbeddingAdapter:
    ...
    def extract_embedding(self, audio_data: bytes, audio_format: str) -> VoiceEmbedding:
        """
    ...
    def _extract_real_embedding(
        ...
        try:
            ...
            waveform = (
                torch.tensor(waveform, dtype=torch.float32).unsqueeze(0).to(self.device)
            )
    ...
    def validate_audio_quality(
        ...
        try:
            ...
            if format_lower not in ["wav", "mp3", "flac", "m4a", "webm", "ogg"]:
                return {
                    "is_valid": False,
                    "reason": f"Unsupported format: {audio_format}",
                }
        ...
        try:
            ...
            with wave.open(audio_file, "rb") as wav_file:
                ...
                if channels > 2:
                    return {
                        "is_valid": False,
                        "reason": f"Too many channels: {channels}",
                    }
                ...
                if sample_rate < 8000:
                    return {
                        "is_valid": False,
                        "reason": f"Sample rate too low: {sample_rate}Hz",
                    }
