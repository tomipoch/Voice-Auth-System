...
class ModelManager:
    ...
    def _download_speechbrain_model(
        self, config: ModelConfig, model_path: Path
    ) -> bool:
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    f"Model {config.name} is not publicly available or requires authentication. Skipping download."
                )
    ...
    def _download_speechbrain_asr_model(
        self, config: ModelConfig, model_path: Path
    ) -> bool:
    ...
    def get_cached_model(self, model_id: str) -> Optional[Any]:
        ...
        with self._cache_lock:
            if model_id in self._model_cache:
                ...
                logger.debug(
                    f"Model {model_id} retrieved from cache (access #{cache_entry.access_count})"
                )
    ...
    def cache_model(self, model_id: str, model_instance: Any) -> bool:
        ...
        if not self._check_memory_available(estimated_size):
            ...
            if not self._check_memory_available(estimated_size):
                logger.warning(
                    f"Still insufficient memory for {model_id}, skipping cache"
                )
                return False
    ...
    def get_memory_usage(self) -> Dict[str, float]:
        ...
        with self._cache_lock:
            total_cache_mb = sum(
                entry.memory_size_mb for entry in self._model_cache.values()
            )
    ...
    def _cleanup_cache(self):
        ...
        with self._cache_lock:
            ...
            sorted_cache = sorted(
                self._model_cache.items(), key=lambda x: x[1].last_accessed
            )
            ...
            for i in range(cleanup_count):
                ...
                logger.info(
                    f"Removed {model_id} from cache (freed {cache_entry.memory_size_mb}MB)"
                )
        ...
        def cleanup_worker():
            while True:
                try:
                    ...
                    memory_stats = self.get_memory_usage()
                    if (
                        memory_stats["system_usage_percent"]
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "models_downloaded": sum(
                1 for model_id in self.models if self.is_model_available(model_id)
            ),
