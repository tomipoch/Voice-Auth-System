...
class PostgresPhraseQualityRulesRepository(PhraseQualityRulesRepositoryPort):
    ...
    async def get_rules_by_type(
        self, rule_type: str, is_active: bool = True
    ) -> List[Dict[str, Any]]:
    ...
    async def update_rule(
        self, rule_name: str, new_value: float, updated_by: Optional[UUID] = None
    ) -> bool:
        ...
        async with self._pool.acquire() as conn:
            ...
            # Log the update
            logger.info(
                f"Rule '{rule_name}' updated to {new_value} by {updated_by or 'system'}"
            )
    ...
    async def get_rule_value(self, rule_name: str, default: float = 0.0) -> float:
        ...
        if not rule or not rule.get("is_active"):
            logger.warning(
                f"Rule '{rule_name}' not found or inactive, using default: {default}"
            )
            return default
        ...
        try:
            ...
            logger.error(
                f"Error parsing rule value for '{rule_name}': {e}, using default: {default}"
            )
            return default
