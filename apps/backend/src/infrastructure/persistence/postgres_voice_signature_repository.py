...
class PostgresVoiceSignatureRepository(VoiceSignatureRepositoryPort):
    ...
    async def save_voiceprint(self, voiceprint: VoiceSignature) -> None:
        ...
        try:
            logger.info(
                f"Attempting to save voiceprint for user_id={voiceprint.user_id}"
            )
            async with self._pool.acquire() as conn:
                encrypted_embedding = self._encryptor.encrypt(
                ...
                logger.info(
                    f"Successfully saved voiceprint for user_id={voiceprint.user_id}, voiceprint_id={voiceprint.id}"
                )
        except Exception as e:
            logger.error(
                f"Failed to save voiceprint for user_id={voiceprint.user_id}: {e}",
                exc_info=True,
            )
            raise
        ...
        async with self._pool.acquire() as conn:
            ...
            for row in rows:
                ...
                sample["embedding"] = np.frombuffer(
                    decrypted_embedding_bytes, dtype=np.float32
                )
