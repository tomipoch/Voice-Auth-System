...
class PostgresChallengeRepository(ChallengeRepositoryPort):
    ...
    async def create_challenge(
        self, user_id: UserId, phrase: str, phrase_id: UUID, expires_at: datetime
    ) -> ChallengeId:
        ...
        async with self._pool.acquire() as conn:
            ...
            logger.info(
                f"Created challenge {challenge_id} for user {user_id} with phrase {phrase_id}"
            )
            return challenge_id
    ...
    async def cleanup_expired_challenges(self, older_than_hours: int = 1) -> int:
        ...
        async with self._pool.acquire() as conn:
            ...
            if count > 0:
                logger.info(
                    f"Cleaned up {count} expired challenges older than {older_than_hours}h"
                )
    ...
    async def cleanup_used_challenges(self, older_than_hours: int = 24) -> int:
        ...
        async with self._pool.acquire() as conn:
            ...
            if count > 0:
                logger.info(
                    f"Cleaned up {count} used challenges older than {older_than_hours}h"
                )
    ...
    async def count_recent_challenges(
        self, user_id: UserId, since_hours: int = 1
    ) -> int:
        """
