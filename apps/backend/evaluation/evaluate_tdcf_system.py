...
class TandemDCFEvaluator:
    """Evaluador con t-DCF, End-to-End FRR y Matriz de Cascada."""
    ...
    def cascade_decision(
        ...
        Cada etapa puede rechazar el audio.
        """
        result = {
        ...
        result['stage1_passed'] = spoof_score < self.thresholds['antispoof']
        ...
        result['stage2_passed'] = identity_score >= self.thresholds['identity']
        ...
        if expected_text is not None:
            ...
            result['stage3_passed'] = text_wer < self.thresholds['text_wer']
            
            if not result['stage3_passed']:
                result['rejection_stage'] = 3
                return result
        else:
            # Sin texto esperado, la etapa 3 se considera pasada
    ...
    def evaluate_system(self):
        """Evaluar sistema completo con t-DCF, End-to-End FRR y Matriz de Cascada."""
        logger.info("=" * 80)
        logger.info("🎯 EVALUACIÓN DEL SISTEMA INTEGRADO: t-DCF + End-to-End FRR + Cascada")
        logger.info("=" * 80)
        logger.info("")
        ...
        cascade_matrix = {
            'genuine': {'stage1': 0, 'stage2': 0, 'stage3': 0, 'accepted': 0},
            'tts': {'stage1': 0, 'stage2': 0, 'stage3': 0, 'accepted': 0},
            'cloning': {'stage1': 0, 'stage2': 0, 'stage3': 0, 'accepted': 0}
        }
        ...
        for username, enrollments in user_enrollments.items():
            ...
            for audio_file in verification_audios:
                ...
                # Registrar scores para t-DCF
                genuine_antispoof_scores.append(result['stage1_antispoof_score'])
                
                # Actualizar matriz de cascada
                if result['final_decision']:
                    cascade_matrix['genuine']['accepted'] += 1
                elif result['rejection_stage'] == 1:
                    cascade_matrix['genuine']['stage1'] += 1
                elif result['rejection_stage'] == 2:
                    cascade_matrix['genuine']['stage2'] += 1
                elif result['rejection_stage'] == 3:
                    cascade_matrix['genuine']['stage3'] += 1
        ...
        for user_dir in attacks_dir.iterdir():
            ...
            for attack_audio in user_dir.glob("*.wav"):
                ...
                # Registrar scores para t-DCF
                attack_antispoof_scores.append(result['stage1_antispoof_score'])
                
                # Actualizar matriz de cascada
                if result['final_decision']:
                    cascade_matrix['tts']['accepted'] += 1
                elif result['rejection_stage'] == 1:
                    cascade_matrix['tts']['stage1'] += 1
                elif result['rejection_stage'] == 2:
                    cascade_matrix['tts']['stage2'] += 1
        ...
        for user_dir in cloning_dir.iterdir():
            ...
            for attack_audio in user_dir.glob("*.wav"):
                ...
                if result['final_decision']:
                    ...
                    cascade_matrix['cloning']['stage1'] += 1
                elif result['rejection_stage'] == 2:
                    cascade_matrix['cloning']['stage2'] += 1
                elif result['rejection_stage'] == 3:
                    cascade_matrix['cloning']['stage3'] += 1
        ...
        logger.info(f"   t-DCF (threshold actual {self.thresholds['antispoof']:.3f}): {tdcf_current['tdcf']:.4f}")
        ...
        # 3. MATRIZ DE DECISIÓN TRIPLE (Cascada)
        logger.info("3️⃣  MATRIZ DE DECISIÓN TRIPLE (Lógica de Cascada)")
        logger.info("")
        logger.info("   GENUINOS:")
        logger.info(f"     Rechazados en Etapa 1 (Antispoofing): {cascade_matrix['genuine']['stage1']}/{total_genuine} ({cascade_matrix['genuine']['stage1']/total_genuine*100:.1f}%)")
        logger.info(f"     Rechazados en Etapa 2 (Speaker): {cascade_matrix['genuine']['stage2']}/{total_genuine} ({cascade_matrix['genuine']['stage2']/total_genuine*100:.1f}%)")
        logger.info(f"     Rechazados en Etapa 3 (Texto): {cascade_matrix['genuine']['stage3']}/{total_genuine} ({cascade_matrix['genuine']['stage3']/total_genuine*100:.1f}%)")
        ...
        logger.info(f"     Rechazados en Etapa 1 (Antispoofing): {cascade_matrix['tts']['stage1']}/{total_tts} ({cascade_matrix['tts']['stage1']/total_tts*100:.1f}%)")
        logger.info(f"     Rechazados en Etapa 2 (Speaker): {cascade_matrix['tts']['stage2']}/{total_tts} ({cascade_matrix['tts']['stage2']/total_tts*100:.1f}%)")
        ...
        logger.info(f"     Rechazados en Etapa 1 (Antispoofing): {cascade_matrix['cloning']['stage1']}/{total_cloning} ({cascade_matrix['cloning']['stage1']/total_cloning*100:.1f}%)")
        logger.info(f"     Rechazados en Etapa 2 (Speaker): {cascade_matrix['cloning']['stage2']}/{total_cloning} ({cascade_matrix['cloning']['stage2']/total_cloning*100:.1f}%)")
        logger.info(f"     Rechazados en Etapa 3 (Texto): {cascade_matrix['cloning']['stage3']}/{total_cloning} ({cascade_matrix['cloning']['stage3']/total_cloning*100:.1f}%)")
        ...
        json_file = results_dir / "tdcf_system_evaluation.json"
    ...
    def plot_results(self, results: Dict, results_dir: Path):
        ...
        # Guardar
        plot_file = results_dir / "tdcf_system_evaluation.png"
