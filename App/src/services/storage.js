...
export const storageUtils = {
  /**
   * Exporta toda la configuración (para backup)
   */
  exportData() {
    ...
    keys.forEach(key => {
      data[key] = storageService.getItem(key);
    });
  ...
  /**
   * Importa configuración (para restore)
   */
  ...
  /**
   * Limpia datos obsoletos basados en timestamp
   */
  cleanupOldData(maxAgeMs = 7 * 24 * 60 * 60 * 1000) { // 7 días por defecto
    const now = Date.now();
    const keys = storageService.getAllKeys();
    let cleaned = 0;
    ...
    if (cleaned > 0) {
      logger.info(`Cleaned up ${cleaned} old storage items`);
    }
