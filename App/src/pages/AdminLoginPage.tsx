...
const AdminLoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Después de login exitoso, el user se actualiza en el contexto
      // Por simplicidad, asumimos que el login fue exitoso si no lanzó error
      // y redirigimos a /admin que manejará la navegación correcta
      navigate('/admin');
    } catch (err: unknown) {
      logger.error('Admin login error:', err);
      const errorMessage =
        err instanceof Error ? err.message : 'Error en el inicio de sesión administrativo';
